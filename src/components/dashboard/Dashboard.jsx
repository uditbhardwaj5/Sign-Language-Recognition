...
const Dashboard = () => {
  ...
  const TopFiveSignsObject = useMemo(() => {
    if (!signDataList.length) return [];
    ...
    return newData.sort((a, b) => b.count - a.count).slice(0, 5);
  }, [signDataList]);
              ...
              <div className="signlang_leader-board">
                ...
                <div className="signlang_toprank-box">
                  {topUsers.map((user, index) => (
                    <div className="signlang_tank-row" key={`user-${user.userId || user.username}-${index}`}>
                ...
                <table>
                  ...
                  <tbody>
                    {TopFiveSignsObject.map((data, i) => (
                      <tr key={`sign-${data.SignDetected}-${i}`} className="sign-row">
                        <td>{i + 1}</td>
